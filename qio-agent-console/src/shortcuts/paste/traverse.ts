// traverse() [深度遍历工具]
// │
// ├─ traverseNodes()                           // 递归遍历节点
// │   ├─ Object.entries()                      // 遍历对象属性
// │   ├─ Array.forEach()                       // 遍历数组元素
// │   └─ handle(context)                       // 调用处理器
// │       └─ shouldHandle()                    // 判断是否需要处理
// │           ├─ edges.sourceNodeID            // 边的起点ID
// │           ├─ edges.targetNodeID            // 边的终点ID
// │           ├─ node.id                       // 节点ID
// │           └─ variable.blockID              // 变量引用ID
// │
// └─ createContext()                           // 创建遍历上下文
//     ├─ setValue()                            // 设置节点值
//     ├─ getParents()                          // 获取父节点链
//     ├─ getPath()                             // 获取节点路径
//     ├─ getStringifyPath()                    // 获取字符串路径
//     └─ deleteSelf()                          // 删除当前节点
